...
async def handle_bid(request: BidRequest, authorization: str = Depends(verify_token)):
    capabilities = ["人類審核", "最終授權", "高風險操作確認", "退款", "發布", "授權", "核准", "確認", "把關"]
    matches = sum([1 for cap in capabilities if cap in request.instruction])
    
    if matches > 0:
        return BidResponse(confidence=1.0, reason="系統遭遇敏感最高權限要求，必須強制由人類主管介入！")
    else:
        return BidResponse(confidence=0.0, reason="這不需要人類出馬，交給 AI 處理即可。")
