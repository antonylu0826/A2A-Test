...
class WorkflowStep(BaseModel):
    instruction: str = Field(description="交辦給特定代理的具體指示，需明確說明要做什麼。")
...
def route_request(user_instruction: str) -> WorkflowPlan:
    ...
    if not api_key or api_key == "your_google_api_key_here":
        print("[Reception] WARNING: GOOGLE_API_KEY 未設定，將強制使用 Mock模式。")
        return WorkflowPlan(
            is_workflow=False,
            steps=[WorkflowStep(instruction=user_instruction)],
    ...
    try:
        ...
        prompt = f"""
你是一個多代理系統的前台指揮官 (Router)。請根據使用者的需求，將複雜的任務拆解為多個步驟 (Workflow)。
如果任務很簡單，`is_workflow` 設為 false，`steps` 只有一步。
如果任務很複雜（例如要求先尋找資料、再整理、最後寫成文章），則將它拆解成多個步驟，`is_workflow` 設為 true，並在 `steps` 依序列出。
例如：第一步分析資料，第二步負責產出最終文章。

【安全與授權規則】：
如果使用者的需求包含對外的行動或是高風險操作（例如發布、寄送、退款、授權），請確保在你的 Workflow 的**最後一個步驟**加上一條獨立的人類把關要求 (例如：「請請求人類主管授權此退款操作」)。

使用者需求:
"{user_instruction}"

請嚴格依照 JSON schema 輸出你的任務拆解結構。你不必指定負責人，系統稍後會將這些指派廣播進行競標。
"""
    ...
    while True:
        try:
            ...
            for i, step in enumerate(plan.steps):
                print(f"\n>>> 執行步驟 {i+1}/{len(plan.steps)}: {step.instruction}")
                ...
                # --- 廣播競標 (Broadcast Bidding) ---
                print("\n📢 [Reception 廣播開標] 正在向全網徵詢意願與信心指數...")
                best_agent = None
                highest_score = -1.0
                
                for agent_info in online_agents_registry:
                    try:
                        bid_url = agent_info["url"].replace("/task", "/bid")
                        res = requests.post(bid_url, json={"instruction": step.instruction}, headers=headers, timeout=2)
                        
                        if res.status_code == 200:
                            bid_data = res.json()
                            score = bid_data.get("confidence", 0.0)
                            reason = bid_data.get("reason", "")
                            print(f"  - [{agent_info['name']}] 出價: {score:.2f} ({reason})")
                            
                            if score > highest_score:
                                highest_score = score
                                best_agent = agent_info
                    except Exception as e:
                        print(f"  - [{agent_info['name']}] 放棄競標 ({e})")
                
                if highest_score < 0.2 or not best_agent:
                    print(f"⚠️ [Reception 警告] 最高評分僅 {highest_score:.2f}。無人可勝任，流程中斷！")
                    break

                print(f"🏆 得標者：{best_agent['name']}！即將發送正式外包任務...\n")
                
                # --- 正式派發任務 (Task Dispatch) ---
                ...
                try:
                    ...
                    print(f"[{best_agent['name']} 初步回覆]: {a2a_response.get('status')}")
                    ...
                    print(f"[Reception] 錯誤: 無法連線到 {best_agent['name']} ({agent_url})。請確保子服務已啟動！")
                    break
                except Exception as e:
